...
import { getWorld, displayName } from "./world.js";
...
可选的角色 id 与特点：
- boss（胡桃）：在后厨/吧台，负责招待和调酒，跟客人熟
- regular（钟离）：坐在角落的常客，话慢，爱点评，跟胡桃很熟
    ...
    const res = await chatWithModel({
      ...
      messages: [
        { role: "system", content: DIRECTOR_SYSTEM },
        {
          role: "user",
          content:
            `在场角色：${present.map(displayName).join("、")}\n` +
            `玩家正在对话的对象：${displayName(speakerId)}\n\n` +
            `最近发生了什么：\n${recent}`,
        },
      ],
    });
